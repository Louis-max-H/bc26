package current.Utils;

import battlecode.common.MapLocation;

public class MapLocationsWithId {
    /**
     * Same functionality of MapLocations but use ID for the hashmap. Recent position will replace old one.
     * ID are mod 4096 (I hope will be enough in most of the case)
     *
     * You can use a publicly available size element on iterate on only (!) these elements.
     * */

    public int maxlen;
    public MapLocation[] locs;
    public char size;
    public char[] hashmap; // = "\uFFFF ..... * 4096"; Defined at end of class
    // Give for an hash, his index in the locs

    public MapLocationsWithId(char len) {
        maxlen = len;
        locs = new MapLocation[maxlen];
        hashmap = new char[4096];
        clear();
    }

    // return boolean false if set is full or already contained location
    public boolean add(MapLocation loc, int id) {
        // Max size
        if (size == maxlen){return false;}

        if(hashmap[id] != '\uFFFF'){
            // Replace already existing element
            locs[hashmap[id]] = loc;
        }else {
            // Add new location
            hashmap[id] = size;
            locs[size] = loc;
            size++;
        }
        return true;
    }

    public boolean add(int xy, int id) {
        // Max size
        if (size == maxlen){return false;}

        if(hashmap[id] != '\uFFFF'){
            // Replace already existing element
            locs[hashmap[id]] = new MapLocation(xy & 0b111111, xy >> 6);
        }else {
            // Add new location
            hashmap[id] = size;
            locs[size] = new MapLocation(xy & 0b111111, xy >> 6);
            size++;
        }
        return true;
    }

    // Check if an element not in set before removing it
    public void remove(int id) {
        // Not in set
        if(hashmap[id] == 0){return;}

        // Replace by last element
        size--;
        char indexRemoved = hashmap[id];
        locs[indexRemoved] = locs[size];
        hashmap[id] = '\uFFFF';

        // Update index of the replacing element
        hashmap[hashmap[size]] = indexRemoved;

        // Okay if we are removing last element of the list
    }

    public boolean contains(int id) {
        return hashmap[id] != '\uFFFF';
    }

    public void clear() {
        size = 0;
        for (int i = 0; i < 4096; i++) {
            hashmap[i] = '\uFFFF';
        }
    }
}
